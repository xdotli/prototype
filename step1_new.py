...
def gradient_descent_U(X, U, V, alpha, lam):
    n = U.shape[0]
    U_new = U - alpha * (2 * np.dot(np.dot(np.dot(X.T, X), U), V.T) - 2 * np.dot(X.T, X) + lam * U)
    return U_new

def gradient_descent_V(X, U, V, alpha, lam):
    V_new = V - alpha * (2 * np.dot(np.dot(np.dot(U.T, X.T), X), V) + lam * V)
    return V_new


def admm(X, lam, alpha, tol=1e-6, max_iter=1000):
    n = X.shape[0]
    U = np.random.randint(100,size=(n,n))
    V = np.random.randint(100,size=(n,n))
